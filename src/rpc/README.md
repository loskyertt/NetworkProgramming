...
普通本地函数调用只需一条 `int result = add(3, 5);`，编译器即可在栈上完成参数传递、跳转和返回。但当被调函数位于另一台机器上时，必须解决以下问题：
...
struct RpcHeader {
  uint32_t magic;        // 魔数：0x52504300 ('RPC0')
...
| 字段 | 类型 | 含义 | 请求 | 响应 |
|---|---|---|---|---|
| `magic` | `uint32_t` | 协议魔数 `0x52504300`，识别 RPC 包 | ✔ | ✔ |
...
> [!warning]
> `#pragma pack(push, 1)` 强制 1 字节对齐。若去掉，编译器会在 `uint8_t` 后插入填充字节，导致 `sizeof(RpcHeader)` 不再是 21 字节，跨编译器/跨平台互通会出错。
...
|   21 字节定长    | svc_len 字节       | meth_len 字节        | payload_len 字节   |
+------------------+--------------------+----------------------+--------------------+
```
...
| `body_size <= RPC_MAX_BODY_SIZE` | 防止恶意长度字段触发超大分配 |
| 请求 `svc_len/meth_len > 0` | 拒绝无名请求 |
| 响应 `svc_len/meth_len == 0` | 拒绝伪装成响应的请求 |

> [!warning]
> ==`RPC_MAX_BODY_SIZE` 当前为 16 MiB==，是单包 body 上限。该限制是防御性设计：恶意或异常的 `payload_len = 0xFFFFFFFF` 会让 `recvFull` 一次性申请 4 GB 内存。
