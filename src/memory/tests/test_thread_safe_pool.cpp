...
static void test_basic_concurrent() {
  ...
  ThreadSafePool pool(OBJ_SIZE, PER_THREAD * NUM_THREADS);
  ...
  for (int t = 0; t < NUM_THREADS; ++t) {
    threads.emplace_back([&, t]() {
      ...
      for (int i = 0; i < PER_THREAD; ++i) {
        ...
        new (mem) Connection(t * PER_THREAD + i, "127.0.0.1");
        ptrs.push_back(mem);
      }
    });
  }
  ...
  for (int t = 0; t < NUM_THREADS; ++t) {
    for (int i = 0; i < PER_THREAD; ++i) {
      ...
      int expected_fd = t * PER_THREAD + i;
...
static void test_concurrent_alloc_dealloc() {
  ...
  ThreadSafePool pool(OBJ_SIZE, 64);
  ...
  for (int t = 0; t < NUM_THREADS; ++t) {
    threads.emplace_back([&, t]() {
      ...
      for (int i = 0; i < PER_THREAD; ++i) {
        void* mem = pool.allocate();
        new (mem) Connection(t * PER_THREAD + i, "192.168.1.1");
        auto* conn = static_cast<Connection*>(mem);

        // 立即验证刚构造的对象
        if (conn->fd != t * PER_THREAD + i || std::strcmp(conn->ip, "192.168.1.1") != 0) {
...
static void bench_concurrent_throughput() {
  ...
  // --- ThreadSafePool ---
  {
    ThreadSafePool pool(OBJ_SIZE, 1024);
    std::vector<std::thread> threads;
    ...
    std::printf("  ThreadSafePool  : %8.2f ms  (%d 线程 × %d alloc+dealloc)\n",
                elapsed_ms(t0, t1), NUM_THREADS, PER_THREAD);
  }
...
static void bench_single_vs_multi() {
  ...
  constexpr int NUM_THREADS     = 4;
  constexpr int PER_THREAD      = TOTAL_OPS / NUM_THREADS;
  constexpr std::size_t OBJ_SIZE = sizeof(Connection);
...
static void bench_expand_stress_multi() {
  ...
  ThreadSafePool pool(OBJ_SIZE, 4);
  ...
  for (int t = 0; t < NUM_THREADS; ++t) {
    threads.emplace_back([&, t]() {
      ...
      for (int i = 0; i < PER_THREAD; ++i) {
        void* mem = pool.allocate();
        new (mem) Connection(t * PER_THREAD + i, "0.0.0.0");
        ptrs.push_back(mem);
      }
    });
  }
  ...
  std::printf("  %d 线程从 4 chunk 扩容到 %d 个对象: %8.2f ms\n",
              NUM_THREADS, NUM_THREADS * PER_THREAD, elapsed_ms(t0, t1));
