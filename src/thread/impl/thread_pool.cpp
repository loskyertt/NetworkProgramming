...
ThreadPool::ThreadPool(int min_thread, int max_thread)
  ...
  // 创建管理者线程
  m_manager = new std::thread(&ThreadPool::do_manage, this);
  // &ThreadPool::do_manage：获取成员函数的指针
  // this：作为隐藏参数传递给成员函数，相当于 this->do_manage()

  // 创建工作线程
  for (int i = 0; i < min_thread; i++) {
    std::thread t(&ThreadPool::do_work, this);
...
ThreadPool::~ThreadPool() {
  ...
  // 这里要避免「调用析构函数的主线程」和「调用 do_manage 的管理者线程」之间存在的竞争窗口
...
void ThreadPool::do_manage(void) {
  ...
  while (m_is_running.load()) {
    ...
    /* lock 作用域（工作线程之间的竞争）：只用于操作队列和检查退出信号 */
    {
      ...
      // 队列为空时才响应退出信号
        ...
        {
          std::lock_guard<std::mutex> lock_ids(m_ids_mutex);
          std::println("---- 工作线程 {} 将要退出（do_work） ...", std::this_thread::get_id());
    ...
    if (task) {
      ...
      if (m_tasks.empty() && m_active_task.load() == 0) { m_done_condition.notify_all(); }
    }
  }
}
